...
SELECT 
    p.prod_id,
    p.prod_name,
    ROUND(p.price, 2) AS standard_price,
    COUNT(DISTINCT od.ord_id) AS orders_sold
FROM products p
LEFT JOIN order_details od ON p.prod_id = od.prod_id
